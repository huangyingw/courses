...
val_batches = get_batches(
    path + 'valid', batch_size=batch_size * 2, shuffle=False)
...
rnd_batches = get_batches(
    path + 'valid', batch_size=batch_size * 2, shuffle=True)
...
val_res = [model.evaluate_generator(
    rnd_batches, rnd_batches.nb_sample) for i in range(10)]
np.round(val_res, 2)
...
model.compile(Adam(lr=10e-5), loss='categorical_crossentropy',
              metrics=['accuracy'])
model.fit_generator(batches, batches.nb_sample, nb_epoch=2, validation_data=val_batches,
                    nb_val_samples=val_batches.nb_sample)
