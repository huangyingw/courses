...
# http://www.statmt.org/wmt10/training-giga-fren.tar. It was created by
# Chris Callison-Burch, who crawled millions of web pages and then used 'a
# set of simple heuristics to transform French URLs onto English URLs
# (i.e. replacing "fr" with "en" and about 40 other hand-written rules),
# and assume that these documents are translations of each other'.
...
# To make this problem a little simpler so we can train our model more
# quickly, we'll just learn to translate questions that begin with 'Wh'
# (e.g. what, why, where which). Here are our regexps that filter the
# sentences we want.
...
# Because it takes a while to load the data, we save the results to make
# it easier to load in later.
...
# Because we are translating at word level, we need to tokenize the text
# first. (Note that it is also possible to translate at character level,
# which doesn't require tokenizing.) There are many tokenizers available,
# but we found we got best results using these simple heuristics.
...
# Special tokens used to pad the end of sentences, and to mark the start
# of a sentence.
...
# Enumerate the unique words (*vocab*) in the corpus, and also create the
# reverse map (word->index). Then use this mapping to encode every
# sentence as a list of int indices.
...
# https://nlp.stanford.edu/projects/glove/ (in the code below we have
# preprocessed them into a bcolz array). We use these because each
# individual word has a single word vector, which is what we need for
# translation. Word2vec, on the other hand, often uses multi-word phrases.
...
en_vecs, en_wv_word, en_wv_idx = load_glove(
    '/data/datasets/nlp/glove/results/6B.100d')
...
# For French word vectors, we're using those from
...
# We need to map each word index in our vocabs to their word vector. Not
# every word in our vocabs will be in our word vectors, since our
# tokenization approach won't be identical to the word vector creators -
# in these cases we simply create a random vector.
...
en_embs, found = create_emb(en_w2v, en_vocab, dim_en_vec)
en_embs.shape, found


fr_embs, found = create_emb(fr_model, fr_vocab, dim_fr_vec)
fr_embs.shape, found


# ## Prep data

# Each sentence has to be of equal length. Keras has a convenient function
# `pad_sequences` to truncate and/or pad each sentence as required - even
# although we're not using keras for the neural net, we can still use any
# functions from it we need!
...
# Here's an example of a French and English sentence, after encoding and
# padding.
...
# Finally, we arrive at a vector representation of the sequence which
# captures everything we need to translate it. We feed this vector into
# more RNN's, which are trying to generate the labels. After this, we make
# a classification for what each word is in the output sequence.
...
# Using *broadcasting* makes a lot of numerical programming far simpler.
# Here's a couple of examples, using numpy:
...
# But Pytorch doesn't support broadcasting. So let's add it to the basic
# operators, and to a general tensor dot product:
...
# Pytorch makes it easy to check intermediate results, when creating a
# custom architecture such as this one, since you can interactively run
# each function.
...
emb_inp = emb(dec_inputs)
emb_inp.size()


w1e = dot(enc_outputs, W1)
w1e.size()


w2h = dot(hidden[-1], W2)
w2h = (w2h + b2.expand_as(w2h)).unsqueeze(1)
w2h.size()
...
res = (res + b3.expand_as(res)).unsqueeze(0)
res.size()


res, hidden = gru(res, hidden)
res.size(), hidden.size()


res = F.log_softmax(out(res.squeeze(0)))
res.size()


# ## Train

# Pytorch has limited functionality for training models automatically -
# you will generally have to write your own training loops. However,
# Pytorch makes it far easier to customize how this training is done, such
# as using *teacher forcing*.
