...
# The first use case of an image to image architecture we're going to look
# at is neural style transfer, using the approach in [this
# paper](https://arxiv.org/abs/1508.06576). This is a fairly popular
# application of deep learning in which an image is recreated in the style
# of a work of art, such as Van Gogh's Starry Night. For more information
# about the use of neural networks in art, see this [Scientific American
# article](https://blogs.scientificamerican.com/sa-visual/neural-networks-for-artists/)
# or [Google's Magenta
...
#
# In order to construct this architecture, we're going to be working with
# keras.backend, which is an abstraction layer that allows us to target
# both theano and tensorflow with the same code.

# The CNN we'll use is VGG16, but with a twist. Previously we've always
# used Vgg with max pooling, and this was useful for image classification.
# It's not as useful in this case however, because max pooling loses
...
# Our result by comparing output at conv 1 of last block (5) is fairly
# amorphous, but still easily recognizable as a bird. Notice that the
...
# If instead we optimized by calculating loss from the output of conv 1 of
# 4th block, our trained image looks much more like the original. This
# makes sense because with less transformations to go through, comparing
...
# One thing to notice is that we're actually going to be calculating the
# loss function multiple layers, rather than just one. (Note however that
...
# The key difference is our choice of loss function. Whereas before we
# were calculating mse of the raw convolutional outputs, here we transform
# them into the "gramian matrix" of their channels (that is, the product
# of a matrix and its transpose) before taking their mse. It's unclear why
# this helps us achieve our goal, but it works. One thought is that the
# gramian  shows how our features at that convolutional layer correlate,
# and completely removes all location information. So matching the gram
...
# Our results are stunning. By transforming the convolutional outputs to
# the gramian, we are somehow able to update the noise pixels to produce
...
# We now know how to reconstruct an image, as well as how to construct an
...
# Like before, we're going to grab a sequence of layer outputs to compute
# the style loss. However, we still only need one layer output to compute
# the content loss. How do we know which layer to grab? As we discussed
# earlier, the lower the layer, the more exact the content reconstruction
# will be. In merging content reconstruction with style, we might expect
# that a looser reconstruction of the content will allow more room for the
# style to have an effect (re: inspiration). Furthermore, a later layer
...
#
# One way for us to tune how the reconstructions mix is by changing the
# factor on the content loss, which we have here as 1/10. If we increase
# that denominator, the style will have a larger effect on the image, and
# if it's too large the original content of the image will be obscured by
...
# There are lots of interesting additional things you could try, such as
# the ideas shown here: https://github.com/titu1994/Neural-Style-Transfer
# .

# ## Use content loss to create a super-resolution network

# So far we've demonstrated how to achieve succesful results in style
# transfer. However, there is an obvious drawback to our implementation,
# namely that we're training an image, not a network, and therefore every
# new image requires us to retrain. It's not a feasible method for any
# sort of real-time application. Fortunately we can adress this issue by
# using a fully convolutional network (FCN), and in particular we'll look
...
# To start we'll define some of the building blocks of our network. In
# particular recall the residual block (as used in
# [Resnet](https://arxiv.org/abs/1512.03385)), which is just a sequence of
# 2 convolutional layers that is added to the initial block input. We also
# have a de-convolutional layer (also known as a "transposed convolution"
# or "fractionally strided convolution"), whose purpose is to learn to
...
#
# The key then is to train this other network to produce an image that
# minimizes the loss between the outputs of some convolutional layer in
# Vgg16 (which the paper refers to as "perceptual loss"). In doing so, we
...
# An important difference in training for super resolution is the loss
...
# Finally we compile this chain of models and we can pass it the original
# low resolution image as well as the high resolution to train on. We also
...
# After training for some time, we get some very impressive results! Look
# at these two images, we can see that the predicted higher resolution
...
#
# Note that we haven't used a test set here, so we don't know if the above
# result is due to over-fitting. As part of your homework, you should
# create a test set, and try to train a model that gets the best result
# you can on the test set.

# ## Fast style transfer

# The original paper showing the above approach to super resolution also
# used this approach to create a much faster style transfer system (for a
# specific style). Take a look at [the
# paper](https://arxiv.org/abs/1603.08155) and the very helpful
# [supplementary
# material](http://cs.stanford.edu/people/jcjohns/papers/eccv16/JohnsonECCV16Supplementary.pdf).
# Your mission, should you choose to accept it, it to modify the super
# resolution example above to do fast style transfer based on this paper.
...
def tot_loss(x):
    ...
    n = len(style_targs)
    for i in range(n):
        loss += mean_sqr_b(gram_matrix_b(x[i + n]) -
                           gram_matrix_b(style_targs[i])) / 2.
