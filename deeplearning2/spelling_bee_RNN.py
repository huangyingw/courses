...
# This notebook starts our deep dive (no pun intended) into NLP by
# introducing sequence-to-sequence learning on Spelling Bee.
...
# Split lines into words, phonemes, convert to indexes (with padding),
...
def eval_keras(input):
    ...
    return (np.mean([all(real == p)
                     for real, p in zip(labels_test, predict)]), predict)
...
#
# The weights for combining all the input states to produce specific
# outputs can be learned using an attentional model; we update the weights
# using SGD, and train it jointly with the encoder/decoder. Once we have
# the outputs, we can classify the character using softmax as usual.
...
#
# This attention model also uses a technique called teacher forcing; in
# addition to passing the encoded hidden state, we also pass the correct
# answer for the previous time period. We give this information to the
# model because it makes it easier to train. In the beginning of training,
# the model will get most things wrong, and if your earlier character
# predictions are wrong then your later ones will likely be as well.
...
hist = model.fit([input_train, dec_input_train], np.expand_dims(labels_train, -1),
                 validation_data=[[input_test, dec_input_test],
